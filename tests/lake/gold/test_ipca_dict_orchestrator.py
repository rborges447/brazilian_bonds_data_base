...
def test_registry_build_ipca_dict_skips_holiday_and_uncovered_dates() -> None:
    out = registry.build(
        "ipca_dict",
        {"ipca_indice": _IPCA, "projecoes": _PROJ},
        BuilderContext(
            dates=["2026-01-01", "2026-05-10"],
            extras={"feriados": {"2026-01-01"}},
        ),
    )
    assert len(out) == 1
...
def test_resolve_feriados_gold_then_silver_fallback() -> None:
    orch = GoldOrchestrator()
    with patch(
        "app.lake.gold.orchestrator.read_feriados_gold",

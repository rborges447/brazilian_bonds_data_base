...
from app.database.schema import LIQUIDACOES_MERCADO_COLUMNS, validate_dataframe_columns
from app.lake.gold.materializers.liquidacoes_mercado import from_silver
...
def test_from_silver_fills_optional_columns_when_absent_in_silver() -> None:
    minimal = {
        "tipo_titulo": "LTN",
        "data_vencimento": "2027-01-01",
        "data_referencia": "2026-01-15",
    ...
    out = from_silver(
        {"liquidacoes_mercado": _liq_df([minimal])},
        BuilderContext(dates=["2026-01-15"]),
    )
    validate_dataframe_columns("LIQUIDACOES_MERCADO", out, LIQUIDACOES_MERCADO_COLUMNS)
    assert out.iloc[0]["status"] == "ATIVO"
    assert pd.isna(out.iloc[0]["expressao"])
    assert pd.isna(out.iloc[0]["codigo_isin"])

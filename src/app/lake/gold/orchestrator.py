...
class GoldOrchestrator:
    ...
    def _month_range_for_ipca(dates: list[str]) -> tuple[str, str]:
        """
        Silver monthly window from DATA_START_DATE (with lookback) through max(dates).

        Lookback of 4 months before the floor supports IPCA fechado M-1 rules.
        """
        floor = pd.Timestamp(get_settings().data_start_date.isoformat()).normalize()
        start = floor - pd.DateOffset(months=4)
        if dates:
        ...
        months = months_in_range(start.strftime("%Y-%m-01"), end_month.strftime("%Y-%m-01"))
        if not months:
            return start.strftime("%Y-%m-01"), end_month.strftime("%Y-%m-01")

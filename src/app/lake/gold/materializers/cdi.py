...
from app.lake.gold.contracts import BuilderContext, CdiGoldValue, SilverFrames
...
def from_silver(silver: SilverFrames, ctx: BuilderContext) -> CdiGoldValue:
    ...
    ``loaded_partitions_cdi`` (typical backfill with ``skip_missing``).
    """
    ...
    if df is None or df.empty:
        raise ValueError(
            f"Silver cdi is empty for loaded partitions: {sorted(enforce_dates)}. "

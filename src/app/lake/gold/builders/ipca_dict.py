...
def projecao_for_as_of(
    df_proj: pd.DataFrame,
    as_of: str | pd.Timestamp,
    **kwargs: object,
) -> pd.Series:
    """
    Projeção para ``as_of``: prefer ``data_coleta <= as_of``, else latest in slice.

    Supports calendar days (weekends/holidays) before the first coleta of the month.
    """
    try:
        return projecao_mais_recente(df_proj, as_of, **kwargs)
    except ValueError:
        sub = df_proj.copy()
        indice = kwargs.get("indice", "IPCA")
        tipo_projecao = kwargs.get("tipo_projecao", "PROJEÇÕES PARA O MÊS CORRENTE")
        ref_month = kwargs.get("ref_month")
        if indice is not None:
            sub = sub[sub["indice"].astype(str).str.upper() == str(indice).upper()]
        if tipo_projecao is not None:
            sub = sub[sub["tipo_projecao"] == tipo_projecao]
        if ref_month is not None:
            mes = pd.Timestamp(ref_month).strftime("%Y-%m-%d")[:7] + "-01"
            sub = sub[sub["ref_month"].astype(str).str[:10] == mes]
        sub["data_coleta"] = pd.to_datetime(sub["data_coleta"], errors="coerce").dt.normalize()
        sub = sub[sub["data_coleta"].notna()]
        if sub.empty:
            raise ValueError(f"Nenhuma projeção no slice para as_of={as_of}")
        return sub.loc[sub["data_coleta"].idxmax()]
...
def projecao_mais_recente_valor(
    ...
    **kwargs: object,
    ...
    row = projecao_for_as_of(df_proj, as_of, **kwargs)
    return (
...
def fechado_ref_months(
    as_of: pd.Timestamp | str,
    data_coleta: pd.Timestamp | str,
) -> tuple[pd.Timestamp, pd.Timestamp]:
    """
    ``ultimo_ref`` / ``anterior_ref`` usados por ``ipca_fechado_from_monthly``.
    ...
    return ultimo_ref.normalize(), anterior_ref.normalize()


def ipca_ref_months_available(
    ipca_monthly: pd.DataFrame,
    ultimo_ref: pd.Timestamp,
    anterior_ref: pd.Timestamp,
) -> bool:
    """True when the sliced monthly frame can satisfy fechado picks."""
    monthly = ipca_monthly.copy()
    monthly["ref_month"] = pd.to_datetime(monthly["ref_month"], errors="coerce").dt.normalize()
    refs = set(monthly["ref_month"].dropna())
    if ultimo_ref.normalize() not in refs:
        return False
    if anterior_ref.normalize() in refs:
        return True
    monthly = monthly.sort_values("ref_month").reset_index(drop=True)
    idx = monthly.index[monthly["ref_month"] == ultimo_ref.normalize()]
    return len(idx) > 0 and int(idx[0]) > 0
...
def ipca_fechado_from_monthly(
    ...
    """IPCA fechado conforme divulgação (ver ``fechado_ref_months``)."""
    ultimo_ref, anterior_ref = fechado_ref_months(as_of, data_coleta)
    div = pd.Timestamp(data_coleta).normalize()
...
def can_build_for_date(
    ...
    *,
    ...
    """
    True when silver monthly history and IPCA ref_months cover ``build_for_date``.

    ``feriados`` is accepted for API symmetry with ``build_for_date``.
    """
    del feriados
    data = pd.Timestamp(as_of).normalize()
    ...
    if ipca_slice.empty or proj_slice.empty:
        return False
    try:
        _, data_coleta_proj = projecao_mais_recente_valor(
            proj_slice, data, respeitar_validade=False
        )
    except (ValueError, KeyError):
        return False
    ultimo_ref, anterior_ref = fechado_ref_months(data, data_coleta_proj)
    return ipca_ref_months_available(ipca_slice, ultimo_ref, anterior_ref)
